...
import { useProducerInputs as UseProducerInputPorts } from '@/components/production-graph/flow/hooks';
...
export const ProducerNode = memo(function ProducerNode({ id, data, selected }: NodeProps<ProducerGraphNode>)
{
	...
	const producer = useProducer({ producerId: producerId ?? undefined });
	const inputs = useProducerInputs({ producerId: producerId ?? undefined });
	const outputs = useProducerOutputs({ producerId: producerId ?? undefined });
