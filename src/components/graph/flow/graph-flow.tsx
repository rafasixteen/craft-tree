...
import { DevTools, graphConfig, layoutGraphNodes } from '@/components/graph';
import { NodeContextMenu, PaneContextMenu } from '@/components/graph/flow/context-menus';
import { GraphEdge, GraphNode } from '@/components/graph/flow/types';
...
export function GraphFlow({ initialNodes, initialEdges, initialViewport, initialTheme }: GraphFlowProps)
{
	...
	const [rfInstance, setRfInstance] = useState<ReactFlowInstance<GraphNode, GraphEdge> | null>(null);
	const { getNodes, getEdges } = useReactFlow<GraphNode, GraphEdge>();
	...
	const onEdgesChange = useCallback(
		function onEdgesChange(changes: EdgeChange<GraphEdge>[])
		{
	...
	const onNodeContextMenu = useCallback(
		function onNodeContextMenu(event: React.MouseEvent, node: GraphNode)
		{
			// Prevent native context menu from showing.
			event.preventDefault();
	...
	const saveGraph = useCallback(
		function saveGraph()
		{
			if (rfInstance)
			{
				updateGraph({ id: graphId, data: rfInstance.toObject() });
				setIsDirty(false);
			}
		},
		[rfInstance, graphId],
	...
	const isValidConnection = useCallback(function isValidConnection(connection: GraphEdge | Connection): boolean
	{
		...
		function hasCycle(node: GraphNode, visited = new Set()): boolean
		{
	...
	{
		const { nodes: layoutedNodes, edges: layoutedEdges } = await layoutGraphNodes(getNodes(), getEdges());
